...
const ORDEN=['portada','cuenta','playa','carta','fotos','galaxia','jardin','juegos','promesas','razones','momentos','cartas-sorpresa','carta-viva','recuerdos','secreto','carta-futuro','playlist','adviento','suenos','album'];
...
// Ella escribe algo y queda guardado en este dispositivo
// (localStorage), para que el regalo también reciba algo de ella.
// ══════════════════════════════════
const CF_KEY = 'carta_futuro_ana_laura';
...
function cfGuardarEnStorage(datos){
  try{ localStorage.setItem(CF_KEY, JSON.stringify(datos)); }
  catch(err){ /* almacenamiento no disponible: no rompemos la página */ }
}

function cfMostrarEstadoInicial(){
  const guardada = cfCargar();
  if(guardada && guardada.texto && guardada.texto.trim()){
    $('cfEscribir').style.display='none';
    $('cfSellada').style.display='block';
    $('cfFechaGuardada').textContent='Escrita el ' + guardada.fecha;
    $('cfTextoGuardado').textContent = guardada.texto;
  } else {
...
if(cfBtnGuardar){
  cfBtnGuardar.addEventListener('click', ()=>{
    ...
    cfGuardarEnStorage({ texto, fecha }); // se guarda ya mismo en este dispositivo, pase lo que pase con el correo
    ...
    emailjs.send('service_anajesus','template_anajesus',{
      ...
      .catch(()=>{
        ...
        cfMostrarEstadoInicial();
        const estado=$('cfEstado');
        if(estado){ estado.textContent='Quedó guardada aquí, aunque el envío por correo falló esta vez.'; estado.classList.add('show'); }
      });
  });
}
...
if(cfBtnEditar){
  cfBtnEditar.addEventListener('click', ()=>{
    const guardada = cfCargar();
    $('cfTexto').value = guardada ? guardada.texto : '';
    $('cfSellada').style.display='none';
    $('cfEscribir').style.display='block';
  });
}
...
// Mostramos el estado correcto (vacío o ya escrita) en cuanto cargue la sección
if($('carta-futuro')){ cfMostrarEstadoInicial(); }
...
// sueños que ella agregue desde la página (se guardan en su
// dispositivo). Cualquiera se puede marcar como cumplido.
// ══════════════════════════════════
const SUENOS_BASE = [
...
const SN_KEY = 'suenos_juntos_estado';

function snCargarEstado(){
  try{
    const raw = localStorage.getItem(SN_KEY);
    return raw ? JSON.parse(raw) : { completados:{}, extras:[] };
  }catch(err){ return { completados:{}, extras:[] }; }
}
function snGuardarEstado(estado){
  try{ localStorage.setItem(SN_KEY, JSON.stringify(estado)); }
  catch(err){ /* si el almacenamiento falla, no rompemos la página */ }
}

function snListaCompleta(){
  const estado = snCargarEstado();
  const base = SUENOS_BASE.map((texto,i)=>({ id:'b'+i, texto, esExtra:false }));
  return [...base, ...estado.extras];
}
...
function snRenderizar(){
  ...
  if(!cont) return;
  const estado = snCargarEstado();
  const items = snListaCompleta();
  cont.innerHTML = items.map(item=>{
    const cumplido = !!estado.completados[item.id];
    return `<div class="sn-item ${cumplido?'cumplido':''}" data-id="${item.id}">
      <button class="sn-check" data-toggle="${item.id}" aria-label="Marcar como cumplido">${cumplido?'✓':''}</button>
  ...
  cont.querySelectorAll('[data-toggle]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      const est = snCargarEstado();
      est.completados[id] = !est.completados[id];
      snGuardarEstado(est);
      snRenderizar();
    });
  });
  cont.querySelectorAll('[data-borrar]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      const est = snCargarEstado();
      est.extras = est.extras.filter(e=>e.id!==id);
      delete est.completados[id];
      snGuardarEstado(est);
      snRenderizar();
    });
  });
...
if(snBtnAgregar){
  snBtnAgregar.addEventListener('click', ()=>{
    ...
    if(!texto) return;
    const est = snCargarEstado();
    est.extras.push({ id:'e'+Date.now(), texto, esExtra:true });
    snGuardarEstado(est);
    input.value='';
    snRenderizar();
  });
